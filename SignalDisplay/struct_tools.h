...
namespace radar
{
	...
	inline bool from_json(const std::string & str, T & t) {
		size_t type_pos = str.find(message_delimiter);
		std::string inc_str = str;
		if (inc_str.substr(0, type_pos).compare(get_type_name(t)) == 0)
			inc_str.erase(0, type_pos + sizeof(message_delimiter));
		try {return x2struct::X::loadjson(inc_str, t, false, false);}
		catch (std::exception & e) {return false;}
	}
	// Fill struct from json stringstream with "StructType:" in the beginning
	template<class T>
	inline bool from_sstream_json(std::stringstream & input_stream, T & t, char end_symbol = '\x00')
	{
		std::string tmp_str;
		...
		std::getline(input_stream, tmp_str, end_symbol);
